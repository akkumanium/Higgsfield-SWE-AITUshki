...
function toUniqueShapeIds(value) {
    const ids = toStringArray(value).map((id) => toShapeId(id, 'shape-ref'));
    return [...new Set(ids)];
}
function buildMediaPlaceholderOperations(arguments_, mediaType) {
    ...
    const statusLabel = status === 'completed'
        ...
        : status === 'failed' || status === 'nsfw'
            ? 'failed'
            : 'pending';
    const terminalLine = errorMessage.length > 0
        ? `\nError: ${errorMessage}`
        : status === 'completed'
            ? '\nStatus: completed'
            : status === 'nsfw'
    ...
    return [
        {
            ...
            payload: {
                ...
                mediaRequest: requestId.length > 0
                    ? {
                        requestId,
                        mediaType,
                        shapeId: id,
...
function mapToolPayloadToOperations(action) {
    ...
    if (toolName === 'cluster_shapes') {
        ...
        const groupId = typeof arguments_.groupId === 'string' && arguments_.groupId.trim().length > 0
