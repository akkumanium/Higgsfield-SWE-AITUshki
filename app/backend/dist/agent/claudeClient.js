...
function buildCanvasContextSummary(request) {
...
// ---------------------------------------------------------------------------
const ALLOWED_LAYOUTS = new Set(['hub', 'linear', 'grid', 'free']);
const ALLOWED_ROLES = new Set(['title', 'point', 'note', 'takeaway']);
...
function sanitizeAndValidatePlan(raw, maxNodes) {
    ...
    const nodes = rawNodes
        ...
        const role = typeof nr.role === 'string' && ALLOWED_ROLES.has(nr.role) ? nr.role : 'point';
        ...
        if (role === 'title') {
            if (titleCount > 0)
                return null; // only one title allowed
            titleCount++;
        }
        seenKeys.add(key);
...
// ---------------------------------------------------------------------------
function inferEdges(plan) {
    if (plan.edges.length > 0)
        return plan.edges; // model provided them, trust it
...
function resolvePositions(plan, viewport, turnId) {
...
// ---------------------------------------------------------------------------
function compilePlanToToolCalls(plan, resolvedNodes, maxTools) {
    const keyToShape = new Map(resolvedNodes.map((n) => [n.key, n]));
    // Stickies first, then arrows — stable order
    const stickyOps = resolvedNodes.map((node) => ({
...
export async function streamGeminiTurn(request) {
    ...
    const enrichedPlan = { ...plan, edges: inferEdges(plan) };
